# Programa de agendamento de contatos que adiciona, remove, edita e lista contatos
...
def adicionar_contato(nome, tel):
    if (len(nome) == 0 or len(tel) == 0):
        return "Necessario informar os dados corretamente!\n"
...
def editar_contato(nome_edit, novo_nome, novo_tel):
    if (len(novo_nome) == 0 or len(novo_tel) == 0 or len(nome_edit) == 0):
        ...
        return f"O contato {nome_editado} foi editado com sucesso!\n"

# Loop principal para manter as opcoes do menu e executar as funcoes
while True:
    print("--" * 10, "Menu de Contatos", "--" * 10)
    exibir_menu()
    ...
    if (escolha_opcao == "1"):
        ...
        print(adicionar_contato(nome_contato, tel_contato))
        time.sleep(1)
    elif (escolha_opcao == "2"):
        nome_remocao = input("Digite o nome do contato a ser removido: ")
        print(remover_contato(nome_remocao))
        time.sleep(1)
        ...
        print(editar_contato(nome_editar, nome_editado, tel_editado))
        time.sleep(1)
